...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      local function auto_import_single()
      ...
      local function auto_import_buffer()
        local bufnr = vim.api.nvim_get_current_buf()
        if vim.bo[bufnr].filetype ~= 'python' then
      ...
      -- Command to auto-import all undefined symbols in buffer
      vim.api.nvim_create_user_command('AutoImportBuffer', auto_import_buffer, { desc = 'Auto-import all undefined symbols in buffer (single options only)' })
