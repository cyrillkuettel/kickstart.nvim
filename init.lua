...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      local python_imports = require 'custom.python_imports'
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- For Python files, this includes smart auto-import behavior
          local function smart_code_action()
            if vim.bo[event.buf].filetype == 'python' then
              -- Try auto-import first for Python files
              python_imports.auto_import_single()
            else
              -- Regular code action for non-Python files
              vim.lsp.buf.code_action()
            end
          end

          map('gra', smart_code_action, '[G]oto Code [A]ction', { 'n', 'x' })
      ...
      vim.api.nvim_create_autocmd('BufWritePre', {
        ...
        pattern = '*.py',
        callback = function(args)
          python_imports.fix_python_imports(args.buf)
        end,
      })
      ...
      vim.api.nvim_create_user_command('FixPythonImports', function()
        python_imports.fix_python_imports(vim.api.nvim_get_current_buf())
      end, { desc = 'Fix Python import ordering' })

      vim.api.nvim_create_user_command(
        'AutoImportBuffer',
        python_imports.auto_import_buffer,
        { desc = 'Auto-import all undefined symbols in buffer (single options only)' }
      )
      ...
      local servers = {
        ...
        -- pyrefly is started via a FileType autocmd above (vim.lsp.start), NOT here.
        -- Adding it here causes a duplicate client alongside the autocmd-started one.
        ruff = {
